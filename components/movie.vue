	...
	<view>
		<uni-collapse-item :title="changemoviename(moviename,number)" showAnimation="true">
			<view style="padding: 10rpx;">
				<uni-card :title="genreChange(moviegenre)" mode="style" 
				:is-shadow="true" :thumbnail="getMovieImg(moviename)"
				 :extra="timestampChange(moviedate,moviecountry)" :note="moviemotto">
					<uni-swipe-action>
						<uni-swipe-action-item :options="options" 
						@click="onClick" @change="change">
							<view style="margin-top: 20rpx;">
								<view style="position: relative;top: 20rpx;">
									<uni-rate style="float: right;position: relative;
									left: 10rpx;" disabled="true" :value="movierate" max="10"
									 size="12">
									</uni-rate>
								</view>
								<view style="float: right;position: relative;
								bottom: 15rpx;left:310rpx">
									<text>{{movierate}}</text>
								</view>
							</view>
				...
				width: 100%;height: 70rpx;position: relative;right: 50rpx;">
					<view>
						<uni-icons type="hand-thumbsup" color='#F5C518' size="20"></uni-icons>
						<uni-icons style="float: right;position: relative;
						left: 10rpx;" type="hand-thumbsdown" color='#000' size="20"></uni-icons>
					</view>
					
					<!-- 删除 -->
					<view style="position: relative;left:100rpx;">
						<uni-fav :checked="true" class="favBtn" circle="true" 
						bg-color-checked='#F5C518' fg-color="#666666"
						 fg-color-checked='#000' bg-color="#eee" @click="startDelete" 
						 :content-text="contentFav"></uni-fav>
					</view>
				</view>
			</view>
		</uni-collapse-item>
